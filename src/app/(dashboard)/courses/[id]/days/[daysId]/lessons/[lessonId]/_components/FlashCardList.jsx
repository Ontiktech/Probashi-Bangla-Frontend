...
const FlashCardList = ({ flashCards, courseId, lessonId, daysId }) => {
  ...
  const handleDelete = async e => {
    ...
    try {
      const response = await deleteFlashCard(flashCardId, daysId, courseId, lessonId)
                ...
                <TableRow key={flashCard?.id}>
                  ...
                  <TableCell align='center'>
                    <Stack direction='row' spacing={1} alignItems='center' justifyContent='center'>
                      <IconButton
                        component={Link}
                        href={`/courses/${courseId}/days/${daysId}/lessons/${lessonId}/flashcard/${flashCard?.id}`}
                      >
                        <i className='ri-eye-fill'></i>
                      </IconButton>
                      <IconButton
                        component={Link}
                        href={`/courses/${courseId}/days/${daysId}/lessons/${lessonId}/flashcard/${flashCard?.id}/update`}
                      >
